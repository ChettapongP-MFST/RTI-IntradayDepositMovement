...
> ⚠️ **Known limitation — "Test connection" in Fabric pipeline UI**
>
> The **Test connection** button on the ADLS Gen2 connection may still return `Forbidden` even after all steps are done correctly. This is a known Fabric UI limitation — the test call does not always go through the Workspace Identity path.
>
> **If the pipeline run itself also fails with Forbidden**, the resource instance rule may not be effective in your tenant/environment. In that case, use the **Workshop workaround** below:
>
> 1. Storage account → **Networking** → set **Public network access scope** to **Enable from all networks**.
> 2. Save and proceed with Workshops 04–09.
> 3. After completing the workshops, you can re-enable the firewall with the ARM template (Option A above) using the specific workspace GUID.
>
> The core learning objectives (pipeline design, KQL ingestion, audit pattern) do not depend on the firewall being locked down.
...
- [ ] **Resource instances** on the Networking blade lists the Fabric workspace — **OR** public network access is enabled (workshop workaround)
