...
| **Ingestion failure** | `wh_control_framework.dbo.ProcessedFiles` — any `Status = 'Failed'` in last 10 min (use a Power BI semantic model measure over the Warehouse as the Activator source, or a scheduled Data Activator query on the Warehouse) | Email on-call + Teams |
